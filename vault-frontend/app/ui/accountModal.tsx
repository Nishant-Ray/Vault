...
export default function AccountModal({ modalType, isOpen, existingNickname, onNicknameSubmit, onRemove, onAddSubmit, onClose }: AccountModalProps) {
  ...
  useEffect(() => {
    if (existingNickname) setNicknameFormState({ nickname: existingNickname });
  }, [existingNickname]);
    ...
    <Modal title={ modalType === ACCOUNT_NICKNAME_MODAL_TYPE ? "Change account nickname" : (modalType === ACCOUNT_REMOVE_MODAL_TYPE ? "Are you sure you want to delete this account?" : "Add an account") } isOpen={isOpen} onClose={handleClose}>
      { modalType === ACCOUNT_NICKNAME_MODAL_TYPE ? (
        ...
        <div className="flex flex-row justify-center gap-8">
          <Button onClick={onRemove} size="md">Delete Account</Button>
